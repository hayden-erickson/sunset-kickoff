
# 🌅 Summer Sunset Yacht Party Series

> **DJs. Live Performances. Sunset Energy. On the Water.**

![Hero Image – Sunset Yacht Party](/img/top-deck-dancing.PNG)

## 🚤 The Party Everyone Will Be Talking About

This summer, we’re turning the lake into a floating festival.

Picture this:

* A luxury yacht cruising at golden hour
* World-class DJs + live performances
* Dancing under string lights as the sun melts into the horizon
* Boats, floats, jet skis, and beach vibes all around

This isn’t just a party.
**It’s a summer memory in the making.**



## 🎥 Watch the Vibe

[▶️ **WATCH THE EVENT VIDEO**](/img/reel-1.MP4)


## 🔥 What Makes This Party Different

* 🎧 **Curated DJs & Live Performers**
* 🌊 **Multi-Level Party Yacht**
* 🌅 **Sunset → Night Cruise Experience**
* 🛟 **Floating Lounge + Water Activities**
* 🕺 **Festival Energy, Ocean Air, Zero Pretension**

Every detail is designed for **maximum joy, movement, and connection**.

![Party Deck at Sunset](/img/behind-DJ.PNG)


## ⏰ Limited Capacity = Real FOMO

This is an **intimate, high-energy experience** — not a packed nightclub.

* ⚠️ **Very limited capacity**
* ⚠️ **Once it’s full, it’s full**
* ⚠️ **Deposits lock your spot**

People *will* be posting about this.
Make sure you’re in the photos — not just liking them later.



## 💸 How It Works

1. **RSVP + Place Your Deposit**
2. Receive confirmation & event details
3. Show up ready to dance, float, and vibe

👉 *Simple. Secure. First-come, first-served.*



## 🚨 Reserve Your Spot Now

> **This will sell out.**
> Don’t wait until your friends text you “we’re on the boat.”

[🟠 **RSVP & SECURE YOUR SPOT**](#)

*(Button links to checkout / payment page)*



![Friends on Floats](/img/bonfire-chat.PNG)



## 🧡 Final Reminder

Summer doesn’t last forever.
**Moments like this don’t repeat.**

If your body already said “yes” —
listen to it.

We’ll see you on the water 🌊✨



### 📍 Event Details

* **Location:** Lake Lanier
* **Time:** Sunset → Night
* **Dress Code:** Swimwear, festival fits, sunset colors encouraged
* **Music:** DJs + Live Performances


